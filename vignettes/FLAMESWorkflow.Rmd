...
if (!file.exists("~/subset_GRCh38.fa")) {
  ...
  R.utils::gunzip("subset_GRCh38.fa.gz", destname = "subset_GRCh38.fa", remove = FALSE, skip = TRUE)
  fa <- "subset_GRCh38.fa"
} else {
  fa <- "~/subset_GRCh38.fa"
}

if (!file.exists("~/sce_lib10")) {
  download.file("https://zenodo.org/records/12770737/files/sce_lib10.qs?download=1")
  download.file("https://zenodo.org/records/12770737/files/sce_lib90.qs?download=1")
...
the gene counts (quantified from the long-reads). Now let's see if we can get
mutations / isoforms specific to each cell line.
...
```{r DTU1, message=FALSE, warning=FALSE}
